-- Tabla para registrar las lecturas semanales de agua del año 2025
...
-- Índices
CREATE INDEX idx_Lecturas_Semana_Agua_2025_numero ON public.Lecturas_Semana_Agua_2025(L_numero_semana);
CREATE INDEX idx_Lecturas_Semana_Agua_2025_fechas ON public.Lecturas_Semana_Agua_2025(L_fecha_inicio, L_fecha_fin);

-- Función para actualizar L_updated_at
CREATE OR REPLACE FUNCTION update_Lecturas_Semana_Agua_2025_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
-- Trigger
CREATE TRIGGER trigger_update_Lecturas_Semana_Agua_2025_updated_at
    BEFORE UPDATE ON public.Lecturas_Semana_Agua_2025
    FOR EACH ROW
    EXECUTE FUNCTION update_Lecturas_Semana_Agua_2025_updated_at();
...
CREATE POLICY "Permitir lectura a usuarios autenticados" 
    ON public.Lecturas_Semana_Agua_2025 FOR SELECT TO authenticated USING (true);

CREATE POLICY "Permitir inserción a usuarios autenticados" 
    ON public.Lecturas_Semana_Agua_2025 FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Permitir actualización a usuarios autenticados" 
    ON public.Lecturas_Semana_Agua_2025 FOR UPDATE TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "Permitir eliminación a usuarios autenticados" 
    ON public.Lecturas_Semana_Agua_2025 FOR DELETE TO authenticated USING (true);
