-- Tabla para registrar las lecturas semanales de agua de todos los puntos de consumo del año 2025
CREATE TABLE IF NOT EXISTS public.lecturas_Semanales_Agua2025 (
...
-- Índices
CREATE INDEX idx_lecturas_Semanales_Agua2025_numero ON public.lecturas_Semanales_Agua2025(L_numero_semana);
CREATE INDEX idx_lecturas_Semanales_Agua2025_fechas ON public.lecturas_Semanales_Agua2025(L_fecha_inicio, L_fecha_fin);

-- Función
CREATE OR REPLACE FUNCTION update_lecturas_Semanales_Agua2025_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
-- Trigger
CREATE TRIGGER trigger_update_lecturas_Semanales_Agua2025_updated_at
    BEFORE UPDATE ON public.lecturas_Semanales_Agua2025
    FOR EACH ROW
    EXECUTE FUNCTION update_lecturas_Semanales_Agua2025_updated_at();

-- Comentarios
COMMENT ON TABLE public.lecturas_Semanales_Agua2025 IS 'Tabla para registrar las lecturas semanales de agua de todos los puntos de consumo del campus - Año 2025';
COMMENT ON COLUMN public.lecturas_Semanales_Agua2025.L_numero_semana IS 'Número de la semana (1, 2, 3, ...)';
COMMENT ON COLUMN public.lecturas_Semanales_Agua2025.L_fecha_inicio IS 'Fecha de inicio de la semana';
COMMENT ON COLUMN public.lecturas_Semanales_Agua2025.L_fecha_fin IS 'Fecha de fin de la semana';

-- RLS
ALTER TABLE public.lecturas_Semanales_Agua2025 ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Permitir lectura a usuarios autenticados" 
    ON public.lecturas_Semanales_Agua2025
    FOR SELECT
    TO authenticated
    USING (true);
