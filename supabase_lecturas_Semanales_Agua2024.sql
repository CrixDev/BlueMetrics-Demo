-- Tabla para registrar las lecturas semanales de agua de todos los puntos de consumo del año 2024
CREATE TABLE IF NOT EXISTS public.lecturas_Semanales_Agua2024 (
...
CREATE INDEX idx_lecturas_Semanales_Agua2024_numero ON public.lecturas_Semanales_Agua2024(L_numero_semana);
CREATE INDEX idx_lecturas_Semanales_Agua2024_fechas ON public.lecturas_Semanales_Agua2024(L_fecha_inicio, L_fecha_fin);

-- Función para actualizar el campo L_updated_at automáticamente
CREATE OR REPLACE FUNCTION update_lecturas_Semanales_Agua2024_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_lecturas_Semanales_Agua2024_updated_at
    BEFORE UPDATE ON public.lecturas_Semanales_Agua2024
    FOR EACH ROW
    EXECUTE FUNCTION update_lecturas_Semanales_Agua2024_updated_at();

-- Comentarios en la tabla
COMMENT ON TABLE public.lecturas_Semanales_Agua2024 IS 'Tabla para registrar las lecturas semanales de agua de todos los puntos de consumo del campus - Año 2024';
COMMENT ON COLUMN public.lecturas_Semanales_Agua2024.L_numero_semana IS 'Número de la semana (1, 2, 3, ...)';
COMMENT ON COLUMN public.lecturas_Semanales_Agua2024.L_fecha_inicio IS 'Fecha de inicio de la semana';
COMMENT ON COLUMN public.lecturas_Semanales_Agua2024.L_fecha_fin IS 'Fecha de fin de la semana';

-- Habilitar Row Level Security (RLS)
ALTER TABLE public.lecturas_Semanales_Agua2024 ENABLE ROW LEVEL SECURITY;

-- Política para permitir lectura a todos los usuarios autenticados
CREATE POLICY "Permitir lectura a usuarios autenticados" 
    ON public.lecturas_Semanales_Agua2024
    FOR SELECT
    TO authenticated
    USING (true);
...
INSERT INTO public.lecturas_Semanales_Agua2024 (
    L_numero_semana, 
    L_fecha_inicio, 
    L_fecha_fin,
    -- Pozos de Servicios
