-- Tabla para registrar las lecturas semanales de todos los puntos de consumo del año 2025
CREATE TABLE IF NOT EXISTS public.lecturas_semana2025 (
...
CREATE INDEX idx_lecturas_semana2025_numero ON public.lecturas_semana2025(numero_semana);
CREATE INDEX idx_lecturas_semana2025_fechas ON public.lecturas_semana2025(fecha_inicio, fecha_fin);

-- Función para actualizar el campo updated_at automáticamente
CREATE OR REPLACE FUNCTION update_lecturas_semana2025_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER trigger_update_lecturas_semana2025_updated_at
    BEFORE UPDATE ON public.lecturas_semana2025
    FOR EACH ROW
    EXECUTE FUNCTION update_lecturas_semana2025_updated_at();

-- Comentarios en la tabla
COMMENT ON TABLE public.lecturas_semana2025 IS 'Tabla para registrar las lecturas semanales de todos los puntos de consumo de agua del campus - Año 2025';
COMMENT ON COLUMN public.lecturas_semana2025.numero_semana IS 'Número de la semana (1, 2, 3, ...)';
COMMENT ON COLUMN public.lecturas_semana2025.fecha_inicio IS 'Fecha de inicio de la semana';
COMMENT ON COLUMN public.lecturas_semana2025.fecha_fin IS 'Fecha de fin de la semana';

-- Habilitar Row Level Security (RLS)
ALTER TABLE public.lecturas_semana2025 ENABLE ROW LEVEL SECURITY;

-- Política para permitir lectura a todos los usuarios autenticados
CREATE POLICY "Permitir lectura a usuarios autenticados" 
    ON public.lecturas_semana2025
    FOR SELECT
    TO authenticated
    USING (true);
...
-- Insertar las primeras dos semanas de ejemplo del año 2025
-- INSERT INTO public.lecturas_semana2025 (numero_semana, fecha_inicio, fecha_fin) VALUES
